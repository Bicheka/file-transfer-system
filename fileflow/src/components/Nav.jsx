...
function Nav(){
    ...
    <nav className="fixed bottom-0 left-0 w-full shadow-[rgba(0,0,15,0.1)_0px_-1px_10px_1px] sm:shadow-[rgba(0,0,15,0.1)_1px_-4px_4px_1px] min-w-20 sm:w-fit bg-[#ffffff]  text-white flex justify-between  sm:relative sm:flex-col sm:h-screen">
      ...
      <div className="flex sm:flex-col justify-around mx-auto sm:mb-16 w-full sm:max-w-fit sm:justify-start size-fit">
          ...
          <NavItem path="/" item={<TbPlugConnected/>} text={"connect"} isActive={location.pathname === "/"}/>

          <NavItem path="/downloads" item={<FaDownload/>} text={"downloads"} isActive={location.pathname === "/downloads"}/>
        
          <NavItem path="/uploads" item={<FaUpload/>} text={"uploads"} isActive={location.pathname === "/uploads"}/>
        
          <NavItem path="/settings" item={<IoMdSettings/>} text={"settings"} isActive={location.pathname === "/settings"}/>
